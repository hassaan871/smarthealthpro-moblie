...
const AppointmentsScreen = () => {
  ...
  const confirmCancellation = async (appointmentId) => {
    try {
      await axios.patch(
        `http://192.168.1.160:5000/appointment/${appointmentId}/cancel`
      );
  ...
  const submitReview = async () => {
    try {
      await axios.post("http://192.168.1.160:5000/review/addReview", {
  ...
  const fetchNotes = async (appointment) => {
    ...
    try {
      ...
      const link = `http://192.168.1.160:5000/user/getMatchingNotes`;
      const response = await axios.get(link, {
        params: {
          doctorId,
          patientId,
  ...
  const fetchPrescriptions = async (appointment) => {
    ...
    try {
      const response = await axios.get(
        `http://192.168.1.160:5000/appointment/${appointment.patient.id}/prescription`
      );
