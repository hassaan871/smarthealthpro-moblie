...
const AppointmentsScreen = () => {
  ...
  const fetchAllAppointments = async () => {
    try {
      ...
      const link = `http://192.168.100.6:5000/appointment/getAllAppointments?${
  ...
  const confirmCancellation = async () => {
    try {
      await axios.delete(
        `http://192.168.100.6:5000/appointment/cancelAppointment/${selectedAppointment._id}`
      );
  ...
  const submitReview = async () => {
    try {
      await axios.post("http://192.168.100.6:5000/review/addReview", {
  ...
  const submitNotes = async () => {
    try {
      ...
      await axios.post("http://192.168.100.6:5000/user/addNotes", {
        appointmentId: selectedAppointment._id,
        note: encryptedNote,
      });
  ...
  const fetchNotes = async (appointment) => {
    ...
    const link = `http://192.168.100.6:5000/user/getUserInfo/${patientId}`;
