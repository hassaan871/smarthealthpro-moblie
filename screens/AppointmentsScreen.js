...
const AppointmentsScreen = () => {
  ...
  const fetchAllAppointments = async () => {
    try {
      ...
      const link = `http://10.135.88.124:5000/appointment/getAllAppointments?${
  ...
  const confirmCancellation = async () => {
    try {
      await axios.delete(
        `http://10.135.88.124:5000/appointment/cancelAppointment/${selectedAppointment._id}`
      );
  ...
  const submitReview = async () => {
    try {
      await axios.post("http://10.135.88.124:5000/review/addReview", {
