...
const ForgotScreen = () => {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.100.135:5000/user/forgot-password', {
        email,
      });
      ...
      Alert.alert('Password Reset', response.data.msg || 'A password reset link has been sent to your email.');
    } catch (error) {
      if (error.response) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.instructions}>Please enter your email address to receive a password reset link.</Text>
