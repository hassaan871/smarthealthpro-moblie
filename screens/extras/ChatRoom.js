...
const ChatRoom = ({ route }) => {
  ...
  useEffect(() => {
    if (convoID && userInfo?._id) {
      axios
        .post(
          `http://192.168.100.5:5000/conversations/${convoID}/mark-messages-read`,
          {
      ...
      axios
        .post(
          `http://192.168.100.5:5000/conversations/${convoID}/read/${userInfo._id}`
        )
        .catch((error) => {
  ...
  const handleMessagePress = async (item) => {
    ...
    if (item.fileInfo && item.fileInfo.mimetype === "application/pdf") {
      try {
        ...
        // Make sure the URL uses http://192.168.100.5:5000 for Android emulator
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const uploadResponse = await axios.post(
          `http://192.168.100.5:5000/upload`,
          formData,
          {
    ...
    if (convoID) {
      try {
        ...
        const response = await axios.get(
          `http://192.168.100.5:5000/conversations/getMessages/${convoID}`
        );
  ...
  const sendNotification = async (receiverId, senderName, messageContent) => {
    try {
      const response = await axios.post(
        "http://192.168.100.5:5000/send-notification",
        {
          receiverId,
  ...
  const sendMessage = async (textMessage = null, fileInfo = null) => {
    ...
    try {
      ...
      const messageResponse = await axios.post(
        `http://192.168.100.5:5000/conversations/${convoID}/messages`,
        newMessage
      );
      ...
      axios
        .put(`http://192.168.100.5:5000/conversations/${convoID}/lastMessage`, {
