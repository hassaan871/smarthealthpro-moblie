...
const HomeScreen2 = () => {
    ...
    const fetchUser = async () => {
      ...
      if (userID !== null) {
        const response = await axios.get(
          `http://192.168.100.34:5000/user/getUserInfo/${userID}`
        );
  ...
  useEffect(() => {
    const fetchAppointment = async () => {
      ...
      if (userID !== null) {
        const response = await axios.get(
          `http://192.168.100.34:5000/appointment/getAllAppointments?PatientId=${userID}`
        );
  ...
  useEffect(() => {
    const fetchPopularDoctors = async () => {
      try {
        const response = await axios.get(
          `http://192.168.100.34:5000/user/getDoctorsBySatisfaction`
        );
        ...
        const popularDoctorsPromises = popularDoctorIds.map((doctor) =>
          axios.get(
            `http://192.168.100.34:5000/user/getDoctorById/${doctor.id}`
          )
        );
